...
def hungarian_algorithm(matrix): 
	...
	for row_num in range(dim): 
		matrix[row_num] = matrix[row_num] - np.min(matrix[row_num])
	
	for col_num in range(dim): 
		matrix[:,col_num] = matrix[:,col_num] - np.min(matrix[:,col_num])
		
	zero_count = 0
	while zero_count < dim:
		ans_pos, marked_rows, marked_cols = mark_matrix(matrix)
		zero_count = len(marked_rows) + len(marked_cols)

		if zero_count < dim:
			matrix = adjust_matrix(matrix, marked_rows, marked_cols)
...
if __name__ == '__main__':
	main()
view raw Hungarian_algorithm.py hosted with ❤ by GitHub