...
class MatchRate(Metric):
    def calculate(self, ride_assignments: list[Ride]) -> None:
        self.value = np.mean(
            list(map(lambda x: int(x.status == RideStatus.COMPLETED), ride_assignments))
        )
...
class CancelRate(Metric):
    def calculate(self, ride_assignments: list[Ride]) -> None:
        self.value = np.mean(
            list(map(lambda x: int(x.status == RideStatus.CANCELLED), ride_assignments))
        )
...
class RideDistributionByDriver(Metric):
    ...
    def calculate(self, ride_assignments: list[Ride]) -> None:
        ...
        rides_by_driver = list(
            map(lambda x: rides_by_driver_dict[x[0]], enumerate(self.drivers))
        )
