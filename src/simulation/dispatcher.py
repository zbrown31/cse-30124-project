...
class Dispatcher:
    def __init__(
        ...
        self.online_drivers_by_id = dict(
            list(zip(self.online_drivers.values(), self.online_drivers.keys()))
        )
    ...
    def simulate_rides(self) -> list[Ride]:
        ...
        while len(resolved_rides) < len(self.ride_requests):
            ...
            # print(current_time)
            while (
                ride_start_index < len(self.ride_requests)
                and self.ride_requests[ride_start_index].request_time < current_time
            ):
                requested_rides.append(self.ride_requests[ride_start_index])
                requested_rides[-1].set_cancel_time(
                    self.canceller.get_cancel_time(current_time)
                )
                ride_start_index += 1

            driver_assignments = self.strategy.assign_drivers(
                requested_rides, self.online_drivers, current_time
            )
            ...
            for assignment in driver_assignments:
                ...
                if driver.current_ride is not None:
                    time_to_pickup = self.mapper.get_distance(
                        driver.current_ride.trip.destination.coordinates,
                        ride.trip.start.coordinates,
                    ).duration
                else:
                    time_to_pickup = timedelta(
                        seconds=(60 * 7) * (np.random.standard_normal() ** 2)
                    )
            ...
            for i, ride in reversed(list(enumerate(active_rides))):
                ...
                if (
                    ...
                    and ride.match_time is not None
                    and ride.time_to_pickup is not None
                    and current_time - ride.match_time >= ride.time_to_pickup
                ):
                    ride.picked_up(current_time)
                if (
                    ride.status == RideStatus.IN_RIDE
                    and ride.pickup_time is not None
                    and current_time - ride.pickup_time >= ride.trip.norm.duration
                ):
...
class Strategy(ABC):
    ...
    def evaluate(
        self, metrics: list[Metric], rides: list[Ride], drivers: list[Driver]
    ) -> list[Metric]:
        self.mapper.initialize_cache(
            list(map(lambda x: (x.trip.start, x.trip.destination), rides))
        )
        dispatcher = Dispatcher(
            rides, drivers, self, ChiCanceller(10 * 60, 5 * 60), self.mapper
        )
