...
class DataReader:
    ...
    def parse_location_json(location_dict: dict[str, Hashable]) -> Location | None:
        try:
            return Location(
                name=str(location_dict["name"]),
                address=str(location_dict["address"]),
                coordinates=Coordinates(
                    round(
                        int(location_dict["coordinates"]["value"]["_latitude"]),
                        PRECISION,
                    ),
                    round(
                        int(location_dict["coordinates"]["value"]["_longitude"]),
                        PRECISION,
                    ),
                ),
            )
    ...
    def parse_trip_json(trip_dict: dict[str, Hashable]) -> Trip | None:
        try:
            return Trip(
                start=DataReader.parse_location_json(trip_dict["start_location"]),
                destination=DataReader.parse_location_json(
                    trip_dict["destination_location"]
                ),
                norm=Norm(
                    trip_dict["distance"],
                    timedelta(seconds=int(trip_dict["estimated_time"])),
                ),
            )
        except KeyError as ex:
            print(f"{ex}")
    ...
    def parse_ride_json(ride_dict: dict[str, Hashable]) -> Ride | None:
        if not "request_time" in ride_dict:
            return None
        return Ride(
            trip=DataReader.parse_trip_json(trip_dict=ride_dict.get("trip", {})),
            price=ride_dict.get("price", 0),
            request_time=datetime.fromtimestamp(
                ride_dict["request_time"]["value"]["_seconds"]
            ),
        )
