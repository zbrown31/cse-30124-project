...
class GMapsClient:
    def __init__(self) -> None:
        ...
        if os.path.exists(
            "/Users/zachbrown/Desktop/School/College/Intro to AI/Project/data/distance_cache.json"
        ):
            with open(
                "/Users/zachbrown/Desktop/School/College/Intro to AI/Project/data/distance_cache.json"
            ) as fh:
    ...
    def get_distance(self, start: Coordinates, destination: Coordinates) -> Norm:
        ...
        if distances["status"] != "ZERO_RESULTS":
            return Norm(
                ...
                timedelta(seconds=distances["duration"]["value"]),
            )
        else:
    ...
    def initialize_cache(self, location_pairs: list[tuple[Location, Location]]):
        ...
        for start, destination in location_pairs:
            ...
            try:
                ...
                self.cache[
                    str(hash((start.coordinates, destination.coordinates)))
                ] = norm
                self.cache[
                    str(hash((destination.coordinates, start.coordinates)))
                ] = norm
            except Exception as ex:
                print(
                    f"Failed to get distance between {start.name} and {destination.name}"
                )
                print(ex)
